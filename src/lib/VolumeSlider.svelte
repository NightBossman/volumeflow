<script>
  ...
  let { value = $bindable(50), label = "Aplikacja", isMaster = false, muted = false, onchange, onmute } = $props();
  function handleInput(e) { value = parseInt(e.target.value); if (onchange) onchange(); }
  function toggleMute() { if (onmute) onmute(); }
...
<div class="slider-container" class:muted>
  <div class="header">
    <div class="label-group">
      <button class="mute-btn" onclick={toggleMute} title={muted ? "Odwycisz" : "Wycisz"}>
        {#if muted}<VolumeX size={16} strokeWidth={2.5} color="#ff4444" />{:else if isMaster}<Volume2 size={16} strokeWidth={2.5} color={isOverdrive ? '#ff4d00' : 'var(--primary-color)'} />{:else}<AppWindow size={16} strokeWidth={2} color="var(--text-secondary)" />{/if}
      </button>
  ...
  <div class="input-wrapper">
    <input type="range" min="0" max="150" {value} oninput={handleInput} disabled={muted} class:master={isMaster} class:overdrive={isOverdrive} />
...
<style>
  ...
  .slider-container.muted { opacity: 0.6; }
  ...
  .mute-btn { background: transparent; border: none; padding: 0; cursor: pointer; display: flex; align-items: center; transition: transform 0.1s; }
  ...
  input[type="range"]:disabled::-webkit-slider-thumb { background: #444; border-color: #666; }
  ...
  .track-fill.is-muted { background: #444; }
